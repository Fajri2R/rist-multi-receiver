...
const runningProcs = {}; // streamId -> { receiver, sender }
...
function buildSenderCmd(stream) {
    ...
    const listenUrl = `rist://@:${stream.forwardPort}?cname=${stream.streamId}`;
...
function startStream(stream) {
    ...
    try {
        ...
        const handleOutput = (d) => {
            ...
            outBuffer = lines.pop(); // Sisa bagian (partial string)

            for (const line of lines) {
                // Cetak log terminal
                process.stdout.write(`[REC:${stream.streamId.slice(0,8)}] ${line}\n`);
                // Deteksi format json stats di output log (fallback bulletproof)
        ...
        recProc.on('error', (err) => console.warn(`[ProcessManager] Warning: '${rec.cmd}' could not be spawned.`));
    } catch (e) {
    ...
    const timer = setTimeout(() => {
        try {
            sndProc = spawn(snd.cmd, snd.args, { stdio: 'pipe' });
            ...
            sndProc.on('error', (err) => {});
            if (runningProcs[stream.streamId]) {
                runningProcs[stream.streamId].sender = sndProc;
            }
        } catch (e) {}
    }, 2000);

    runningProcs[stream.streamId] = { receiver: recProc, sender: null, timer };
    console.log(`[ProcessManager] Configured stream ${stream.streamId.slice(0,8)} [Recv:${stream.receivePort} -> Fwd:${stream.forwardPort}]`);
}
